...
def _try_load_spacy_model(model_name: str, disable: Iterable[str] = ()):
    disable = set(disable)
    logger.info("Trying to load model {}".format(model_name))
    try:
        if model_name == 'ru2':
            #model = ru2.load_ru2(model_name)
...
@register('stream_spacy_tokenizer_only')
class StreamSpacyTokenizerOnly(Component):
    ...
    def __init__(self, disable: Optional[Iterable[str]] = None, stopwords: Optional[List[str]] = None,
        ...
        self.modelinit_spacy_model=spacy_model
        self.modelinit_disable=disable
        ...
        _ngram_range = ngram_range or self.ngram_range
         for i, doc in enumerate(
            try:
                ...
                gc.collect(1)
                gc.collect(2)
                self.model = _try_load_spacy_model(self.modelinit_spacy_model, disable=self.modelinit_disable)
                self.model.pipe(data, batch_size=_batch_size)):
